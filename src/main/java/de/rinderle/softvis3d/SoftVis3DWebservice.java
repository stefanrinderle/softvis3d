...
public class SoftVis3DWebservice implements WebService {
  ...
  public SoftVis3DWebservice(final DatabaseSession session,
		...
		final DependencyDao dependencyDao = softVis3DInjector
				.getInstance(DependencyDao.class);
	...
	public void define(final Context context) {
		final WebService.NewController controller = context
				.createController("api/softVis3D");
		controller.setDescription("SoftVis3D webservice");

		// create the URL /api/softVis3D/getTree
		controller.createAction("getTree").setDescription("Get tree structure")
