...
public class BottomUpProcessorTest {
	...
	//
	// verify(this.resourceTreeService, times(1)).getChildrenLeafIds(MAP_KEY,
	// snapshotId);
	// verify(this.resourceTreeService, times(1)).getChildrenNodeIds(MAP_KEY,
	// snapshotId);
	// }
	//
	// @Test
	...
	// childrenTreeNodes.add(new TreeNode(2, null, depth, TreeNodeType.TREE,
	// "2", METRIC_FOOTPRINT, METRIC_HEIGHT));
	// childrenTreeNodes.add(new TreeNode(2, null, depth, TreeNodeType.TREE,
	// "3", METRIC_FOOTPRINT, METRIC_HEIGHT));
	// when(this.resourceTreeService.getChildrenNodeIds(MAP_KEY,
	// snapshotId)).thenReturn(childrenTreeNodes);
	//
	// this.underTest.accept(this.mockVisitor, snapshotId, MAP_KEY);
	//
	// // verify(this.resourceTreeService,
	// times(3)).getChildrenLeafIds(eq(MAP_KEY), eq(snapshotId));
	// // verify(this.resourceTreeService,
	// times(3)).getChildrenNodeIds(eq(MAP_KEY), eq(snapshotId));
	// }
	//
	// @Test
	...
	// childrenTreeLeaves.add(new TreeNode(2, null, depth, TreeNodeType.TREE,
	// "2", METRIC_FOOTPRINT, METRIC_HEIGHT));
	// childrenTreeLeaves.add(new TreeNode(2, null, depth, TreeNodeType.TREE,
	// "3", METRIC_FOOTPRINT, METRIC_HEIGHT));
	// when(this.resourceTreeService.getChildrenLeafIds(MAP_KEY,
	// snapshotId)).thenReturn(childrenTreeLeaves);
	//
	// this.underTest.accept(this.mockVisitor, snapshotId, MAP_KEY);
	//
	// verify(this.resourceTreeService,
	// times(1)).getChildrenNodeIds(eq(MAP_KEY), eq(snapshotId));
	// verify(this.resourceTreeService,
	// times(1)).getChildrenLeafIds(eq(MAP_KEY), eq(snapshotId));
	// }
