...
public class SnapshotVisitorBeanTest {
	...
	public void testVisitNodeMinimal() throws DotExecutorException {
		...
		// when(dotExecutor.run(any(Graph.class),
		// any(Settings.class))).thenReturn(graph);

		final Settings settings = new Settings();
		final List<Double> minMaxValues = this.createMinMaxValues();
		// SnapshotVisitorImpl underTest = new SnapshotVisitorImpl(formatter,
		// dotExecutor, settings, minMaxValues);

		// SonarSnapshot snapshot = new SonarSnapshot(ID, NAME, DEPTH,
		// METRIC_FOOTPRINT_VALUE, METRIC_HEIGHT_VALUE);
		final List<LayeredLayoutElement> elements = new ArrayList<LayeredLayoutElement>();

		// LayeredLayoutElement result = underTest.visitNode(snapshot,
		// elements);
		//
		// assertEquals(LayeredLayoutElement.Type.NODE,
	...
	// doThrow(DotExcecutorException.class).when(dotExecutor).run(any(Graph.class),
	// eq(settings));
	//
	...
	// SonarSnapshot snapshot = new SonarSnapshot(ID, NAME, DEPTH,
	// METRIC_FOOTPRINT_VALUE, METRIC_HEIGHT_VALUE);
	// List<LayeredLayoutElement> elements = new
	// ArrayList<LayeredLayoutElement>();
	...
	public void testVisitLeaf() {
		...
		//
		// assertEquals(LayeredLayoutElement.Type.LEAF,
		// result.getElementType());
		// assertEquals(ID, result.getId());
		// assertEquals(NAME, result.getDisplayName());
		//
		// verify(formatter,
		// times(1)).calcBuildingHeight(eq(METRIC_HEIGHT_VALUE),
		// any(SonarMetric.class));
		// verify(formatter,
		// times(1)).calcSideLength(eq(METRIC_FOOTPRINT_VALUE),
		// any(SonarMetric.class));
		//
		// Map<Integer, Graph> graphResult = underTest.getResultingGraphList();
	...
	// createLayeredLayoutLeafElement(snapshot, sideLength, sideLength,
	// buildingHeight);
	// }
