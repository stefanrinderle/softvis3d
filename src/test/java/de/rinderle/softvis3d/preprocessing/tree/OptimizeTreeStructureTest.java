...
public class OptimizeTreeStructureTest {
    ...
    public void testNormalizeWithLongPath() {
        ...
        assertNotNull("next child after main is not softVis3D",
                tree.getChildren().get("src").getChildren().get("main/java/de/rinderle/softVis3D"));
        assertEquals("depth is not right", Integer.valueOf(2),
                tree.getChildren().get("src").getChildren().get("main/java/de/rinderle/softVis3D").getDepth());
        assertNotNull("next child after main is not softVis3D",
                tree.getChildren().get("src").getChildren().get("test/java/de/rinderle/softVis3D"));
        assertEquals("depth is not right", Integer.valueOf(2),
                tree.getChildren().get("src").getChildren().get("test/java/de/rinderle/softVis3D").getDepth());

        assertEquals(
                "depth is not right",
                Integer.valueOf(3),
                tree.getChildren().get("src").getChildren().get("main/java/de/rinderle/softVis3D").getChildren()
                        .get("guice").getDepth());
    }
