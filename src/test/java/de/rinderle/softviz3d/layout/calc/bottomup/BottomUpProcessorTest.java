...
public class BottomUpProcessorTest {
  ...
  public void setUp() {
    ...
    // underTest = new BottomUpProcessor(resourceTreeService);
  }
  ...
  public void testChildrenNodes() throws DotExcecutorException {
    ...
    // when(sonarService.getSnapshotsByIds(eq(childrenTreeNodeIds), eq(METRIC_FOOTPRINT), eq(METRIC_HEIGHT),
    // eq(depth))).thenReturn(snapshots);
  ...
  public void testChildrenLeaves() throws DotExcecutorException {
    ...
    // snapshots.add(createSonarSnapshot(2));
    // snapshots.add(createSonarSnapshot(3));
    // when(sonarService.getSnapshotsByIds(eq(childrenLeafIds), eq(METRIC_FOOTPRINT), eq(METRIC_HEIGHT), eq(depth +
    // 1))).thenReturn(snapshots);
