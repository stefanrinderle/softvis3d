...
} from "../../src/constants/ColorMetrics";
...
describe("ColorMetrics", () => {
    ...
    it("should find layout by id", () => {
        expect(ColorMetrics.isScmColorMetric(coverageColorMetric)).to.be.eq(false);

        expect(ColorMetrics.isScmColorMetric(leakPeriodCommitsScmColorMetric)).to.be.eq(true);
        expect(ColorMetrics.isScmColorMetric(numberOfAuthorsScmColorMetric)).to.be.eq(true);
    });
});
