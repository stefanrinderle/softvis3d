...
BASEDIR="/Users/stefan/opt/sonarqube-4.5.2/"
...
'init')
    ...
    sed -i '' 's/^\(sonar\.jdbc\.username\s*=\s*\).*$/\sonar\.jdbc\.username=sonar452/' ${RUNNERCONFIG}
    sed -i '' 's/^\(sonar\.jdbc\.password\s*=\s*\).*$/\sonar\.jdbc\.password=sonar452/' ${RUNNERCONFIG}
    sed -i '' 's/^\(sonar\.jdbc\.url=\s*\).*$/\sonar\.jdbc\.url=jdbc:mysql:\/\/localhost:3306\/sonar452?useUnicode=true\&amp;characterEncoding=utf8/' ${RUNNERCONFIG}
    ;;
'dev')
