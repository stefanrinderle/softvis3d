...
BASEDIR="/Users/stefan/opt/sonarqube-5.0.1/"
...
'init')
    ...
    sed -i '' 's/^\(sonar\.jdbc\.username\s*=\s*\).*$/\sonar\.jdbc\.username=sonar501/' ${RUNNERCONFIG}
    sed -i '' 's/^\(sonar\.jdbc\.password\s*=\s*\).*$/\sonar\.jdbc\.password=sonar501/' ${RUNNERCONFIG}
    sed -i '' 's/^\(sonar\.jdbc\.url=\s*\).*$/\sonar\.jdbc\.url=jdbc:mysql:\/\/localhost:3306\/sonar501?useUnicode=true\&amp;characterEncoding=utf8/' ${RUNNERCONFIG}
    ;;
'dev')
